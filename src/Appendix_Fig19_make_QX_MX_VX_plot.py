...
def main():
    ...
    axes2 = [axes[0].twinx(), axes[1].twinx(), axes[2].twinx()]
    idx2y_max = defaultdict(float)
    idx2y_max2 = defaultdict(float)
    idx2y_label = {0: r'$Q(X)$', 1: r'$M(X)$', 2: r'$V(X)$'}
    ...
    for mx, (model_name, layer_idx2token_stats) in enumerate(
        ...
        for idx, ys in enumerate([Q_X_list, M_X_list, V_X_list]):
    ...
    save_path = output_dir / 'QX_MX_VX_plot.pdf'
